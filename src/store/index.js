...
// 日付（YYYY-MM-DD）に基づく連番伝票番号を生成（例：DN-260609-01）
const generateDateBasedSlipNo = (dateStr) => {
  if (!dateStr) dateStr = new Date().toISOString().split('T')[0];
  const dateObj = new Date(dateStr);
  const yy = String(dateObj.getFullYear()).slice(2);
  const mm = String(dateObj.getMonth() + 1).padStart(2, '0');
  const dd = String(dateObj.getDate()).padStart(2, '0');
  const prefix = `DN-${yy}${mm}${dd}-`;
  
  const notes = state.records.t_delivery_note || [];
  let maxNum = 0;
  notes.forEach(n => {
    if (n.slipNo && n.slipNo.startsWith(prefix)) {
      const numStr = n.slipNo.replace(prefix, '');
...
export const actions = {
  ...
  addDeliveryNote(record) {
    ...
    const slipNo = record.slipNo || generateDateBasedSlipNo(targetDate);
  ...
  bulkAssignSlipNumbers() {
    ...
    unassigned.forEach(note => {
      ...
      if (idx !== -1) {
        const targetDate = state.records.t_delivery_note[idx].date || new Date().toISOString().split('T')[0];
        state.records.t_delivery_note[idx] = {
          ...state.records.t_delivery_note[idx],
          slipNo: generateDateBasedSlipNo(targetDate)
        };
      }
    });
  ...
  /**
   * 【移行用】全ての伝票番号を新しい日付ベースのフォーマット（DN-YYMMDD-XX）で振り直す
   * ※実行すると既存の伝票番号が上書きされます。
   */
  migrateAllSlipNumbersToDateBased() {
    if (!confirm('全ての納品書の伝票番号を「日付順（DN-YYMMDD-XX）」に振り直します。よろしいですか？\n※一度振り直すと元には戻せません。')) {
      return;
    }
    
    const notes = state.records.t_delivery_note || [];
    // 日付昇順で並び替えてから採番する（古いものから01, 02...となるように）
    const sortedNotes = [...notes].sort((a, b) => (a.date || '').localeCompare(b.date || ''));
    
    // 一旦既存の slipNo をクリアする（generateDateBasedSlipNo が重複検知しないようにするため）
    sortedNotes.forEach(note => {
      const idx = state.records.t_delivery_note.findIndex(n => n.id === note.id);
      if (idx !== -1) {
        state.records.t_delivery_note[idx].slipNo = null;
      }
    });
    ...
    this.syncToCloud();
    this.showToast(`全 ${sortedNotes.length} 件の伝票番号を新しいフォーマットに振り直しました`, 'success');
  },
